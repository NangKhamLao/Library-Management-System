...
func (h *Handler) createBookings(rw http.ResponseWriter, r *http.Request) {
	...
	var book Book
	h.db.Debug().Raw("select * from books where id = ?", i).Scan(&book)
...
func (h *Handler) storeBookings(rw http.ResponseWriter, r *http.Request) {
	...
	const insertBooking = `INSERT INTO bookings(user_id,book_id,Start_time,end_time) VALUES(?,?,?,?)`
	res := h.db.Debug().Exec(insertBooking, 1, booking.BookID, booking.Start_time, booking.End_time)
	getBook := `UPDATE books SET status = false WHERE id = ?`
	h.db.Debug().Exec(getBook, booking.BookID)
...
func (h *Handler) myBookings(rw http.ResponseWriter, r *http.Request) {
	...
	h.db.Debug().Raw("SELECT * FROM bookings  limit ? offset ?", limit, offset).Scan(&booking)
	total := 0
	h.db.Debug().Raw("select count(*) from bookings").Scan(&total)
	nextPageURL := ""
	previousPageURL := ""
	...
	for key, value := range booking {
		var book Book
		h.db.Debug().Raw("select book_name from books where id = ?", value.BookID).Scan(&book)
