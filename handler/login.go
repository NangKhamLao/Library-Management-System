...
func (h *Handler) loginCheck(rw http.ResponseWriter, r *http.Request) {
	...
	var user SignUp
	email := ""
	h.db.Debug().Raw("select email from users where email = ?", login.Email).Scan(&email)
	if email == "" {
	...
	password := ""
	h.db.Debug().Raw("select password from users where email = ?", login.Email).Scan(&password)
