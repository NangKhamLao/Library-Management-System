...
func (h *Handler) storeCategories(rw http.ResponseWriter, r *http.Request) {
	...
	const insertCategory = `INSERT INTO categories(name,status) VALUES(?,?)`
	res := h.db.Debug().Exec(insertCategory, category.Name, category.Status)

	if res.RowsAffected == 0 {
		http.Error(rw, "insert failed", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) listCategories(rw http.ResponseWriter, r *http.Request) {
	...
	nextPageURL := ""
	previousPageURL := ""
	h.db.Debug().Raw(`SELECT count(*) FROM categories`).Scan(&total)
	h.db.Debug().Raw("SELECT * FROM categories  limit ? offset ?", limit, offset).Scan(&category)
...
func (h *Handler) editCategories(rw http.ResponseWriter, r *http.Request) {
	...
	var category Category
	h.db.Debug().Raw(getCategory, id).Scan(&category)
...
func (h *Handler) updateCategories(rw http.ResponseWriter, r *http.Request) {
	...
	const updateCategories = `UPDATE categories SET name = ?, status = ? WHERE id = ?`
	res := h.db.Debug().Exec(updateCategories, id, cat.Name, cat.Status)
	if res.RowsAffected == 0 {
		http.Error(rw, "update failed", http.StatusInternalServerError)
		return
	}
	http.Redirect(rw, r, "/category/list", http.StatusTemporaryRedirect)
}
...
func (h *Handler) deleteCategories(rw http.ResponseWriter, r *http.Request) {
	...
	const deleteCategories = `DELETE FROM categories WHERE id = ?`
	res := h.db.Debug().Exec(deleteCategories, id)
	if res.RowsAffected == 0 {
		http.Error(rw, "invalid URL", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) searchCategory(rw http.ResponseWriter, r *http.Request) {
	...
	const getSearch = "SELECT * FROM categories WHERE name LIKE ?"
	category := []Category{}
	h.db.Debug().Raw(getSearch, "%"+cat+"%").Scan(&category)
	list := ListCategory{
		Categories: category,
	}
	if err := h.templates.ExecuteTemplate(rw, "list-category.html", list); err != nil {
